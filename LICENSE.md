# Licensing

Copyright 2026 Julian Y. Richard Corbet.

The current sporewright library is licensed under **LGPL-3.0-only WITH
LGPL-3.0-linking-exception**. The complete
[LGPL version 3](LICENSES/LGPL-3.0-only.txt) incorporates
[GPL version 3](LICENSES/GPL-3.0-only.txt); the
[linking exception](<LICENSES/LGPL-3.0-only WITH LGPL-3.0-linking-exception.txt>) waives the
Minimal Corresponding Source, Minimal Application Code and
installation-information duties (LGPLv3 §§4d/4e) for combined works that
link statically or dynamically.

Applications may use the library under different licenses subject to the LGPL's
conditions. Distribution must preserve the library's notices and the openness
of library modifications, including reverse engineering to debug such changes.
The application license must not restrict those library rights. No relinking
route or object files are required for combined works under the exception.

Dependencies and external executables retain their own licenses and service
terms. The library's license does not grant rights to an external runtime or
subscription. Preserve applicable upstream copyright, license and NOTICE files
when distributing those components (see [THIRD-PARTY.md](THIRD-PARTY.md)).

Previously distributed source retains its original grants: versions published
before this relicense remain available under their original terms (FSL-1.1-ALv2
with automatic Apache-2.0 conversion after two years, and Apache-2.0 for the
earliest release). Repository copies of those grants are retained at
`LICENSES/FSL-1.1-ALv2.txt` and `LICENSES/Apache-2.0.txt` for historical
reference and excluded from the current crate package. They are not alternative
grants for new LGPL-covered work. Old tags keep their old grants — the new
license rides the new `0.3.0` version identity (see [CHANGELOG.md](CHANGELOG.md)).
Existing commits and artifacts are not rewritten.

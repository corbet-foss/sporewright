 ...
 *
 * Chain keys name the override tier a preference lives at:
 *   prefix                        L0 (stage)     stage-level defaults
 *   prefix:consumerId             L1 (consumer)  consumer-specific override
 *   prefix:consumerId:instanceId  L2 (instance)  per-job/per-section override
 *
 * Stage prefix = stageId, except evaluate → 'eval'.
 *
 * These are pure address helpers. The cascade *order* — the (provider, model)
 * fallback chain — is not computed here: it emerges from a `resolve` over the
 * override-tier sporewright tensor (see `cascade-tensor.ts`). This module is the

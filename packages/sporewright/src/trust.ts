...
export function updatePairTrust(input: {
  ...
  agree: boolean;
  freshness: number;
  /** Learned source volatility in [0,1]. It discounts disagreement only. */
  sourceVolatility?: number;
}): PairTrustResult {
  const volatility = clamp01(input.sourceVolatility ?? 0);
  const base = input.agree
    ? TRUST_AGREE_BASE_GAIN
    : -TRUST_DISAGREE_BASE_LOSS * (1 - volatility);
  return {

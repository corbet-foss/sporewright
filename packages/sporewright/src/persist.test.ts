...
import { MemStore, Tensor } from "./index";

test("a snapshot survives a store", () => {
  const a = new Tensor(["global", "device"]);
  a.writer("device")!.setValue("device", "A", "groq", "latency", 0.1);
...
test("cross-core: a Rust-shaped snapshot restores in TS", () => {
  const snap = '{"levels":["global","device"],"cells":[{"level":"device","inst":"A","option":"groq","dim":"latency","v":0.1}]}';

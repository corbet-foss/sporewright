...
import { Budget, budgetDim, DEFAULT_STEP, MAX_LAMBDA, scope, Tensor, TokenBucket } from "./index";
    ...
    for (let win = 0; win < 8; win++) {
      ...
      for (let j = 0; j < 30; j++) {
        budget.writeUsage(
          feeder.writer("job")!,
          "job",
          scope({ window: "agg", job: `j${j}` }),
          "hot",
          1.0,
        );
      }
      const measured = budget.aggregateUsage(feeder, "job", "window", scope({ window: "agg" }));
...
describe("usage aggregation (reduceSum feeder)", () => {
  it("sums every draw on a shared pool across instances and options", () => {
      ...
      .mapOption("m2", "acct");
    {
      ...
      budget.writeUsage(w, "job", scope({ job: "j1" }), "m1", 10.0);
      budget.writeUsage(w, "job", scope({ job: "j2" }), "m1", 20.0);
      budget.writeUsage(w, "job", scope({ job: "j3" }), "m2", 5.0);
    }
    const m = budget.aggregateUsage(t, "job", "global", {});
  ...
  it("a non-finite usage writes nothing and keeps the option routable", () => {
    ...
    {
      const w = t.writer("job")!;
      const job = scope({ job: "j1" });
      w.setWeight("job", {}, "financial", 1.0);
      w.setValue("job", job, "m", "financial", 0.0);
      expect(budget.writeUsage(w, "job", job, "m", NaN)).toBe(false);
      expect(budget.writeUsage(w, "job", job, "m", Infinity)).toBe(false);
    }

...
/** Compare two strings by Unicode CODE POINT (not UTF-16 code unit) — identical to
 *  Rust's `String::cmp` (UTF-8 byte order) over the full Unicode range. Mirrors
 *  `scmp` in `./tensor` (kept local so the tensor module's public surface stays
 *  unchanged): the per-option pool totals must accumulate in Rust's `BTreeMap`
 *  (sorted-by-name) order, and the naïve `<` comparator disagrees with Rust on
 *  astral-plane names. Load-bearing for cross-core decision-equivalence. */
...
export class Budget {
  ...
  writeUsage(
    ...
    const e = writer.setValue(level, inst, option, budgetDim(pool), usage);
    if (e) return e;
    return true;
  }
   ...
   *  `undefined`. */
  publishPrices(writer: Writer, level: string): WriteError | undefined {
    for (const [pool, p] of this.pools) {
      ...
      const e = writer.setWeight(level, SHARED, budgetDim(pool), lambda);
      if (e) return e;
    }
    return undefined;
  }
  ...
  aggregateUsage(t: Tensor, fromLevel: string, toLevel: string, toInst: string): Map<string, number> {
    ...
    for (const pool of this.pools.keys()) {
      ...
      // produce the OPPOSITE queue. So we sort the contributing options first, by
      // Unicode code point (matching Rust's byte order over the full range, astral
      // plane included — the same comparator `Tensor` uses for dims/pairs).
      // Same discipline as `Tensor.dimsOf`/`reduceSum`.
      const opts: string[] = [];
      for (const [option, optPool] of this.optionPool) {
        if (optPool === pool) opts.push(option);
      }
      opts.sort(cmpCodePoint);
      let total = 0;
      for (const option of opts) {
        const u = t.reduceSum(fromLevel, toLevel, toInst, option, dim);
        if (u !== undefined && Number.isFinite(u)) total += u;
      }

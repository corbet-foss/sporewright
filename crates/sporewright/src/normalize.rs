...
mod tests {
    ...
    fn normalization_flips_a_lexicographic_order_into_a_balanced_one() {
        ...
        let mut t = Tensor::new(["global"]);
        {
            ...
            w.set_value("global", "", "a", "financial", 0.002).unwrap();
            w.set_value("global", "", "a", "latency", 5.0).unwrap();
            w.set_value("global", "", "b", "financial", 0.010).unwrap();
            w.set_value("global", "", "b", "latency", 0.5).unwrap();
        }
            ...
            .with_scale("latency", 5.0);
        {
            ...
            .unwrap();
            w.set_weight("global", "", "latency", n.effective_weight("latency", 1.0))
                .unwrap();
        }
        assert_eq!(t.resolve(&Cursor::new()), vec!["b", "a"]);

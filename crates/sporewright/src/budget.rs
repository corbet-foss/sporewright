...
use crate::tensor::{Tensor, WriteError, Writer, SHARED};
...
///
/// **No derived `Default`**: a derived `Default` would set `step = 0.0`, which [`Budget::step`]
/// floors to [`MIN_STEP`] (`1e-6`) — a 100,000× slower price reaction than the
/// [`DEFAULT_STEP`] (`0.1`) that [`Budget::new`] installs. That would silently diverge
/// from the TS core (whose field initializer always runs `DEFAULT_STEP`). The manual
/// [`Default`] impl below delegates to [`Budget::new`], so `Budget::default()` and
/// `Budget::new()` are identical; prefer spelling `new()` at call sites.
#[derive(Clone, Debug, PartialEq)]
pub struct Budget {
...
impl Default for Budget {
    /// Identical to [`Budget::new`] (never a zero step — see the type docs).
    fn default() -> Self {
        Self::new()
    }
}
...
impl Budget {
    ...
    pub fn write_usage(
        ...
        writer.set_value(level, inst, option, &dim, usage)?;
        Ok(true)
    }
    ...
    pub fn publish_prices(&self, writer: &mut Writer<'_>, level: &str) -> Result<(), WriteError> {
        for (pool, p) in &self.pools {
            ...
            writer.set_weight(level, SHARED, &dim, lambda)?;
        }
        Ok(())
    }
    ...
    pub fn aggregate_usage(
        ...
        for pool in self.pools.keys() {
            ...
            for (option, opt_pool) in &self.option_pool {
                ...
                if let Some(u) = t.reduce_sum(from_level, to_level, to_inst, option, &dim) {
                    if u.is_finite() {
                        total += u;
                    }
                }
...
mod tests {
    ...
    fn price_is_a_shared_weight_not_the_gas_pedal() {
        ...
        let mut t = Tensor::new(["global"]);
        {
            ...
            w.set_value("global", "", "free_a", "financial", 0.0)
                .unwrap();
            w.set_value("global", "", "free_b", "financial", 0.0)
                .unwrap();
        }
        ...
        // Each option draws 1 unit per call onto its pool dim.
        {
            let mut w = t.writer("global").unwrap();
            budget
                .write_usage(&mut w, "global", "", "free_a", 1.0)
                .unwrap();
            budget
                .write_usage(&mut w, "global", "", "free_b", 1.0)
                .unwrap();
        }
        // Both idle ⇒ both prices 0 ⇒ tie broken by name.
        {
            let mut w = t.writer("global").unwrap();
            budget.publish_prices(&mut w, "global").unwrap();
        }
        ...
        {
            let mut w = t.writer("global").unwrap();
            saturated.publish_prices(&mut w, "global").unwrap();
        }
    ...
    fn saturation_simulation_load_sheds_to_the_idle_pool() {
        ...
        let mut t = Tensor::new(["global"]);
        {
            let mut w = t.writer("global").unwrap();
            w.set_weight("global", "", "financial", 1.0).unwrap();
            w.set_value("global", "", "hot", "financial", 0.0).unwrap();
            w.set_value("global", "", "cold", "financial", 0.0).unwrap();
            // Each option draws 1 unit on its pool per resolve (the steady-state probe).
            w.set_value("global", "", "hot", &budget_dim("hot_pool"), 1.0)
                .unwrap();
            w.set_value("global", "", "cold", &budget_dim("cold_pool"), 1.0)
                .unwrap();
        }
        ...
        for _win in 0..8 {
            ...
            for j in 0..30 {
                ...
                budget
                    .write_usage(&mut w, "job", &inst, "hot", 1.0)
                    .unwrap();
            }
            let measured = budget.aggregate_usage(&mut feeder, "job", "window", "agg");
    ...
    fn aggregate_usage_sums_draws_across_instances() {
        ...
        // Three jobs draw on the shared account via two options m1, m2.
        {
            ...
            budget.write_usage(&mut w, "job", "j1", "m1", 10.0).unwrap();
            budget.write_usage(&mut w, "job", "j2", "m1", 20.0).unwrap();
            budget.write_usage(&mut w, "job", "j3", "m2", 5.0).unwrap();
        }
        let m = budget.aggregate_usage(&mut t, "job", "global", "agg");
    ...
    fn unmapped_option_carries_no_budget_term() {
        ...
        assert_eq!(
            budget.write_usage(&mut w, "global", "", "lonely", 9.0),
            Ok(false)
        );
    }
    ...
    fn lambda_is_clamped_to_a_finite_ceiling_and_the_saturated_option_sorts_last() {
        ...
        let mut t = Tensor::new(["global"]);
        {
            let mut w = t.writer("global").unwrap();
            w.set_weight("global", "", "financial", 1.0).unwrap();
            w.set_value("global", "", "saturated", "financial", 0.0)
                .unwrap();
            w.set_value("global", "", "rival", "financial", 1.0)
                .unwrap();
            // saturated draws 1 unit on its pool each call.
            w.set_value("global", "", "saturated", &budget_dim("p"), 1.0)
                .unwrap();
        }
        ...
        {
            let mut w = t.writer("global").unwrap();
            b.publish_prices(&mut w, "global").unwrap();
        }
    ...
    fn non_finite_usage_writes_nothing_and_keeps_the_option_routable() {
        ...
        {
            let mut w = t.writer("job").unwrap();
            w.set_weight("job", "", "financial", 1.0).unwrap();
            w.set_value("job", "j1", "m", "financial", 0.0).unwrap();
            // A NaN usage on the mapped option — must write nothing.
            assert_eq!(
                budget.write_usage(&mut w, "job", "j1", "m", f64::NAN),
                Ok(false)
            );
            // An infinite usage likewise.
            assert_eq!(
                budget.write_usage(&mut w, "job", "j1", "m", f64::INFINITY),
                Ok(false)
            );
        }
    ...
    #[test]
    fn default_is_new_never_a_zero_step() {
        // Clippy's new_without_default is satisfied by a manual impl that delegates
        // to new(): a derived Default would install step 0.0 (floored to MIN_STEP,
        // 100,000× slower than DEFAULT_STEP, diverging from the TS core).
        assert_eq!(Budget::default(), Budget::new());
        assert_eq!(Budget::default().step(), DEFAULT_STEP);
    }

...
use sporewright::{Budget, Cursor, Tensor, TokenBucket};
...
fn run_usage_vector(file: &str, vector: &Value) {
    ...
    for write in feeder_spec.get("writes").and_then(Value::as_array).unwrap() {
        ...
        budget
            .write_usage(&mut w, from_level, inst, option, usage)
    ...
    let measured = budget.aggregate_usage(&mut feeder, from_level, to_level, to_inst);

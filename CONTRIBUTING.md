...
every `resolve`/`reduce`/`fold`, and the wire must round-trip between them. Read
...
**decision-equivalence**: given the same facts, both cores must produce the
**identical** `resolve` / `reduce` / `fold` / `corroborate` result — that is what
lets a device (TS) and an orchestrator (Rust), or any mix, agree on the same
queue and the same materialised cells. The serialised wire **round-trips** (each
...
2. Keep the decision identical and the wire round-tripping: field names, the JSON
   cell shape, and the value/weight encodings must line up so each core parses
...
Previously distributed versions remain available under Apache-2.0 (see
[`LICENSES/Apache-2.0.txt`](LICENSES/Apache-2.0.txt) and [`CHANGELOG.md`](CHANGELOG.md)).

To keep the project coherently stewarded, external contributors retain copyright
in their work and grant the rights described in the
